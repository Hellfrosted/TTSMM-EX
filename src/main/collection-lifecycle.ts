...
import { ModCollection } from '../model';
...
function refersToSameCollectionPath(oldpath: string, newpath: string): boolean {
	if (oldpath === newpath) {
		return true;
	}
...
export function executeCollectionLifecycleCommand(userDataPath: string, command: CollectionLifecycleCommand): boolean {
	switch (command.action) {
		case 'create':
			return updateCollectionFile(userDataPath, command.collection);
		case 'rename':
			return renameCollectionFile(userDataPath, command.collection, command.newName);
		case 'delete':
			return deleteCollectionFile(userDataPath, command.collection);
	}
}
