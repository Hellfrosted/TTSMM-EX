...
- Active Tailwind phase: commits 29-39 have established Tailwind conventions, mapped app tokens, converted shared loading primitives, migrated startup/loading screens, converted notification UI, migrated settings form layout, converted collection management modal UI, converted the Block Lookup shell UI, converted the Collection view shell UI, preserved virtualized table internals on custom CSS, and removed obsolete CSS from the migrated groups. Next up is commit 40: visual regression checkpoints for migrated UI.
