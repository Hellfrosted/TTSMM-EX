...
- Active Tailwind phase: commits 29-40 have established Tailwind conventions, mapped app tokens, converted shared loading primitives, migrated startup/loading screens, converted notification UI, migrated settings form layout, converted collection management modal UI, converted the Block Lookup shell UI, converted the Collection view shell UI, preserved virtualized table internals on custom CSS, removed obsolete CSS from the migrated groups, and recorded visual-checkpoint coverage for migrated UI. Extraction phase is active: commits 41-49 have moved main collection table sizing, row presentation, header interactions, Block Lookup table layout helpers, Block Lookup workflow state, mod detail dependency panels, mod detail preview/header presentation, and collection lifecycle command handlers into focused modules, then re-ran the focused renderer extraction tests. Next up is commit 50: update test helpers for React Query and Tailwind conventions.
