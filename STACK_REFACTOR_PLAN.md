...
- Active Tailwind phase: commits 29-40 have established Tailwind conventions, mapped app tokens, converted shared loading primitives, migrated startup/loading screens, converted notification UI, migrated settings form layout, converted collection management modal UI, converted the Block Lookup shell UI, converted the Collection view shell UI, preserved virtualized table internals on custom CSS, removed obsolete CSS from the migrated groups, and recorded visual-checkpoint coverage for migrated UI. Extraction phase is complete through commit 50: commits 41-49 moved main collection, Block Lookup, mod details, and collection lifecycle behavior into focused modules and re-ran focused extraction tests; commit 50 centralized React Query test helpers and reduced class-coupled renderer assertions. Dependency phase is active: commit 51 applied patch/minor dependency updates, commit 52 refreshed the lockfile/dependency-tree checkpoint, commit 53 passed audit plus canonical validation, commit 54 completed practical packaging/Steamworks smoke checks, and commit 55 updated README script documentation. Next up is commit 56: product/developer notes only where behavior changed.
